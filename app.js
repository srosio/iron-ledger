...
const IronLedger = {
     ...
     */
    init() {
        console.log('🚀 SweepAgent initializing...');
        this.loadState();
    ...
    displayHotCoins(coins) {
        ...
        display.innerHTML = coins.map(coin => {
            ...
            const tvLink = `tradingview://chart?symbol=BINANCE:${coin.symbol}.P`;
    ...
    updateTradingViewChart(symbol) {
        ...
        // Update TradingView link (format for mobile app: tradingview://chart?symbol=BINANCE:BTCUSDT.P)
        const tvSymbol = `BINANCE:${symbol}.P`;
        linkElement.href = `tradingview://chart?symbol=${tvSymbol}`;
     ...
     * Uses native share sheet on iOS/Android, Telegram URL on desktop
     */
    async shareToTelegram() {
        ...
        // Plain text version for native share (no markdown)
        let plainMessage = `${icon} ${symbol} Market Analysis\n\n`;
        plainMessage += `${rec.recommendation} BIAS (${rec.confidence} Confidence)\n\n`;

        // Add market data if available
        if (marketData) {
            plainMessage += `Live Data:\n`;
            plainMessage += `💰 Mark Price: $${marketData.markPrice?.toFixed(2) || 'N/A'}\n`;
            plainMessage += `📊 Funding: ${marketData.lastFundingRate?.toFixed(4) || 'N/A'}%\n`;
            plainMessage += `📈 OI: ${marketData.openInterest?.toLocaleString() || 'N/A'}\n`;
            plainMessage += `📉 24h Change: ${marketData.priceChangePercent?.toFixed(2) || 'N/A'}%\n\n`;
        }

        // Add reasoning
        plainMessage += `Analysis:\n`;
        ...
        plainMessage += `\n\nGenerated by SweepAgent`;

        // Check if Web Share API is supported (iOS, Android native share)
        if (navigator.share) {
            try {
                ...
                if (error.name !== 'AbortError') {
                    ...
                    // Fallback to Telegram URL if share fails
                    this.shareToTelegramUrl(plainMessage);
                }
            }
        } else {
            // Desktop or unsupported browser - use Telegram URL
            this.shareToTelegramUrl(plainMessage);
        }

        console.log('📱 Sharing recommendation:', rec);
    },

    /**
     * Fallback method to share via Telegram URL (for desktop)
     */
    shareToTelegramUrl(message) {
